from fastapi import Depends, FastAPI, HTTPException, Query, Form, File, UploadFile, Request, Response
...
async def http_get_repos(current_user: Annotated[User, Depends(get_current_active_user)]):
...
async def http_get_commits(params: GetCommits, current_user: Annotated[User, Depends(get_current_active_user)]) -> List[Commit]:
    r = get_commits(current_user.github_access_token, current_user.username, params.repo, params.branch)
    commits = r.json()
    print(commits)
    return commits
...
async def http_get_commit(params: GetCommit, current_user: Annotated[User, Depends(get_current_active_user)]) -> Commit:
    r = get_commit(current_user.github_access_token, current_user.username, params.repo, params.sha)
    commit = r.json()
...
@app.get("/scan", response_model=List[dict])
async def scan_commit(params: GetCommit, current_user: Annotated[User, Depends(get_current_active_user)]):
    r = get_commit(current_user.github_access_token, current_user.username, params.repo, params.sha)
    commit_json = r.json()
    output_list = []
    if commit_json["files"]:
        ...
        for i in range(files_changed):
            ...
            LLM_request = requests.post(url, json = LLM_query, headers={"Content-Type": "application/json"})
            LLM_response = LLM_request.json()
            LLM_response_text = LLM_response["candidates"][0]["content"]["parts"][0]["text"]
